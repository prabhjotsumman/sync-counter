...
function isCounter(obj: unknown): obj is Counter {
  ...
  return (
    ...
    // dailyGoal, dailyCount, history are optional
  );
}
...
export async function updateCounter(id: string, updates: { name?: string; value?: number; dailyGoal?: number; dailyCount?: number; history?: Counter['history'] }): Promise<Counter | null> {
  if (isLocal && localDbPath) {
    const dbFile = path.resolve(process.cwd(), localDbPath);
